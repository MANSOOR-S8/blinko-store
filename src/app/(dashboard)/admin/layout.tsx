    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-[var(--border-color)] fixed h-full z-20">
        <div className="p-6 border-b border-[var(--border-color)]">
          <Link href="/admin" className="text-2xl font-bold text-[var(--primary-color)]">
            Admin Panel
          </Link>
        </div>
      ...
      {/* Mobile Sidebar */}
      <aside className={`lg:hidden fixed inset-y-0 left-0 w-64 bg-white border-r border-[var(--border-color)] z-50 transform transition-transform duration-300 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-6 border-b border-[var(--border-color)] flex items-center justify-between">
          <Link href="/admin" className="text-2xl font-bold text-[var(--primary-color)]">
            Admin Panel
          </Link>
          <button onClick={() => setIsMobileMenuOpen(false)} className="text-[var(--light-text)]">
            <X size={24} />
          </button>
        </div>
